...
class GymStaffRating: UIControl {
    ...
    override func draw(_ rect: CGRect) {
        ...
        for index in 0...totalStars {
            ...
            let highlighted = (index+1 <= RatingsState.ratings["GymStaff"]!)
...
private extension GymStaffRating{
    ...
    func updateRating(with value: Int){
        ...
        if(self.rating != value && value >= 0 && value <= totalStars){
            self.rating = value
            print(self.rating)
            RatingsState.ratings["GymStaff"] = self.rating
            setNeedsDisplay()
        }
    }
