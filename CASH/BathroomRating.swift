...
class BathroomRating: UIControl {
    ...
    override func draw(_ rect: CGRect) {
        ...
        for index in 0...totalStars {
            ...
            let highlighted = (index+1 <= RatingsState.ratings["Bathroom"]!)
...
private extension BathroomRating{
    ...
    func updateRating(with value: Int){
        ...
        if(self.rating != value && value >= 0 && value <= totalStars){
            self.rating = value
            print(self.rating)
            RatingsState.ratings["Bathroom"] = self.rating
            setNeedsDisplay()
        }
    }
